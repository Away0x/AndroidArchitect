...
import com.away0x.hi.ui.banner.core.HiViewPager;
...
/**
 * HiBanner的控制器
 * 辅助 HiBanner 完成各种功能的控制
 * 将 HiBanner 的一些逻辑内聚在这，保证暴露给使用者的 HiBanner 干净整洁
 */
public class HiBannerDelegate implements ViewPager.OnPageChangeListener, IHiBanner {
    ...
    private IHiIndicator mHiIndicator;
    ...
    public void onPageSelected(int position) {
        ...
        if (mHiIndicator != null) mHiIndicator.onPointChange(position, mAdapter.getRealCount());
    }
    ...
    private void init(@LayoutRes int layoutResId) {
        if (mAdapter == null) mAdapter = new HiBannerAdapter(mContext);
        if (mHiIndicator == null) mHiIndicator = new HiCircleIndicator(mContext);
